...
class Metadata:
    ...
    def get_hardware_metadata(self, sys_data):
        ...
        for item in sys_data['system_data']['machines']:
                if not 'hardware_details' in hard_dict:
                ...
                hardware_dict['os_name'] = item['ansible_distribution']+item['ansible_distribution_version']
    ...
    def get_environment_metadata(self, sys_data):
        env_dict = {}
        for item in sys_data['system_data']['machines']:
            if not 'environment_setup' in env_dict:
    ...
    def get_software_metadata(self, sys_data):
        soft_all_dict = {}
        for item in sys_data['system_data']['machines']:
            if not 'software_details' in soft_all_dict:
            ...
            if any(node in item['inventory_hostname'] for node in nodes):
                if not 'openstack' in soft_all_dict['software_details']:
                    soft_all_dict['software_details']['openstack']={}
                if not 'config' in soft_all_dict['software_details']['openstack']:
                ...
                for soft in item:
                    if 'openstack' in soft:
                        ...
                        if not service_name in software_dict:
